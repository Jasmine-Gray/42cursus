...
static void	run_dda(t_data *d, t_ray *ray)
{
	while (1)
	{
...
static void	set_wall_params(t_data *d, t_ray *ray, t_wall *w)
{
	if (ray->side == 0)
	{
...
void	render_frame(t_data *d)
{
	...
	while (x < WIDTH)
	{
		...
		init_ray(d, d->dir_x + d->plane_x * cam,
			d->dir_y + d->plane_y * cam, &ray);
		run_dda(d, &ray);
		set_wall_params(d, &ray, &wall);
