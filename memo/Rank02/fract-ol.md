...
Here's your comparison table formatted in Markdown:
...
「複素平面上を変化させる」とは、複素数 C または Z の値が複素平面上のさまざまな点に対応して変わっていくことを意味します。複素平面とは実部（x軸）と虚部（y軸）で表される二次元の平面です。

マンデルブロ集合とジュリア集合の違いを具体的に説明すると：

#### マンデルブロ集合の場合：
- 計算を始める初期値 Z₀ は常に 0+0i（原点）に固定されています
- パラメータ C の値を複素平面上の様々な点（例：C = 0.5+0.5i, C = -1.2+0.3i など）に変えながら計算します
- つまり、「複素平面上の各点 C に対して、Z₀=0 から始めて Z=Z²+C を繰り返し計算し、発散するかどうかを調べる」という処理をしています
- 発散しない点 C の集合がマンデルブロ集合となります
...
簡単に言えば、マンデルブロ集合では C を変化させ Z₀ を固定、ジュリア集合では C を固定し Z₀ を変化させるということです。この違いによって、両者は異なる形状のフラクタル画像を生成します。
